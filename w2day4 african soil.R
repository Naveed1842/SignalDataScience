...
examine_test = data.frame(read_csv("Predictions.csv"))
head(examine_test)
dim(predictions)
dim(examine_test)
...
write_csv(predictions2, "Predictions2.csv", col_names = TRUE)
...
logP.train =train(x=bands, y=P.df[['logP']], method="glmnet", tuneGrid=grid_list[['P']], trControl=control)
...
write_csv(predictions3, "Predictions3.csv", col_names = TRUE)


# For our third attempt, we can take the results from our second attempt, and average them with a different method



# Define Naive Bayes model

subset_df = cbind(unscaled_bands, targets['Ca'])
head(subset_df[,1:6])
subset_df[1:10,'Ca']
NB_model = naiveBayes(Ca~., data = subset_df)
NB_prediction = predict(NB_model, newdata = unscaled_test_bands)
